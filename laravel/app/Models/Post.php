...
class Post extends Model
{
    ...
    public function author_info(){
        ...
        // $users  = User::where('admin_id',$admin_id)->get(); // المستخدمسن المرتبظين بالتاجر الحالي
        // dd($users);
        // return $this->hasOne('App\User','id','added_by');
        return $this->hasOne('App\User','id','added_by')->where('admin_id',$admin_id);
    }
    ...
    public static function countActivePost(){
        ...
        $users_id  = User::where('admin_id',$admin_id)->pluck('id');
        $data=Post::whereIn('added_by',$users_id)->where('status','active')->count();
        if($data){
            return $data;
        }
        return 0;
