                    ...
                    <div class="right-content">
                        <ul class="list-main">
                            <li><i class="ti-location-pin"></i> <a
                                    href="{{ route('order.track') }}">{{ trans('main_header.track_order') }}</a>
                            </li>
                            {{-- <li><i class="ti-alarm-clock"></i> <a href="#">Daily deal</a></li> --}}
                            @if (auth('trader')->check() || auth()->check()) {{-- @auth('trader') --}}

                                {{-- @if (Auth::user()->role == 'admin' || auth()->guard('trader')->check()) --}}
                                @if (auth('trader')->check())
                                    <li><i class="ti-user"></i> <a href="{{ route('admin') }}"
                                            target="_blank">{{ trans('main_header.dashboard') }}</a>
                                    </li>
                                @endif

                                {{-- إخغاء الداشبورد اذا كان المسجل مستخدم عادي --}}
                                {{-- @else
                                    <li><i class="ti-user"></i> <a href="{{ route('user') }}" target="_blank">{{trans('main_header.dashboard')}}</a>
                                    </li> --}}
                                {{-- @endif --}}
                                <li><i class="ti-power-off"></i> <a
                                        href="{{ route('user.logout') }}">{{ trans('main_header.logout') }}</a></li>
                            @else
                                <li><i class="ti-power-off"></i><a
                                        href="{{ route('login.form') }}">{{ trans('main_header.login') }} /</a> <a
                                        href="{{ route('register.form') }}">{{ trans('main_header.register') }}</a>
                                </li>
                            @endif
                        </ul>
                    </div>
                ...
                <div class="col-lg-8 col-md-7 col-12">
                    <div class="search-bar-top">
                        <div class="search-bar">
                            <select>
                                <option>{{ trans('main_header.all_cat') }}</option>
                            ...
                            <form method="POST" action="{{ route('product.search') }}">
                                @csrf
                                <input name="search" placeholder="{{ trans('main_header.search_prod') }}....."
                                    type="search">
                                    ...
                                    <div class="nav-inner">
                                        <ul class="nav main-menu menu navbar-nav">
                                            <li class="{{ Request::path() == 'home' ? 'active' : '' }}"><a
                                                    href="{{ route('home') }}">{{ trans('main_header.home') }}</a>
                                            </li>
                                            <li class="{{ Request::path() == 'about-us' ? 'active' : '' }}"><a
                                                    href="{{ route('about-us') }}">{{ trans('main_header.about_us') }}</a>
                                            </li>
                                            <li class="@if (Request::path() == 'product-grids' || Request::path() == 'product-lists') active @endif"><a
                                                    href="{{ route('product-grids') }}">{{ trans('main_header.products') }}</a><span
                                                    class="new">{{ trans('main_header.new') }}</span></li>
                                            {{ Helper::getHeaderCategory() }}
                                            <li class="{{ Request::path() == 'blog' ? 'active' : '' }}"><a
                                                    href="{{ route('blog') }}">{{ trans('main_header.blog') }}</a>
                                            </li>

                                            <li class="{{ Request::path() == 'contact' ? 'active' : '' }}"><a
                                                    href="{{ route('contact') }}">{{ trans('main_header.contact_us') }}</a>
                                            </li>
                                        </ul>
                                    </div>
                                </div>
