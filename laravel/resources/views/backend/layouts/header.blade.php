<nav class="navbar navbar-expand navbar-light bg-white topbar mb-4 static-top shadow">
    ...
    {{-- <a href="{{ route('show.store') }}" class="btn btn-sm mr-10">
        show
    </a> --}}
    ...
    <ul class="navbar-nav ml-auto">
        ...
        <li class="nav-item dropdown no-arrow mx-1">
            <a class="nav-link dropdown-toggle" href="{{ route('show.store') }}" target="_blank" data-toggle="tooltip"
        ...
        <li class="nav-item dropdown no-arrow">
            <a class="nav-link dropdown-toggle" href="#" id="userDropdown" role="button" data-toggle="dropdown"
                ...
                <span
                    class="mr-2 d-none d-lg-inline text-gray-600 small">{{ auth()->guard('trader')->user()->name }}</span>
                {{-- @if (Auth()->user()->photo) --}}
                @if (auth()->guard('trader')->user()->photo)
                    {{-- <img class="img-profile rounded-circle" src="{{ Auth()->user()->photo }}"> --}}
                    <img class="img-profile rounded-circle" src="{{ auth()->guard('trader')->user()->photo }}">
                @else
            ...
            <div class="dropdown-menu dropdown-menu-right shadow animated--grow-in" aria-labelledby="userDropdown">
                ...
                <a class="dropdown-item" href="{{ route('updatae.storeInfo') }}">
