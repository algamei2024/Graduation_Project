<div id="notifications">
    <a class="nav-link dropdown-toggle" href="#" id="alertsDropdown" role="button" data-toggle="dropdown"
        ...
        <span class="badge badge-danger badge-counter">
            ...
            @if (count(auth()->guard('trader')->user()->unreadNotifications) > 5)
            ...
            @else
                <span class="count" {{-- data-count="{{ count(Auth::user()->unreadNotifications) }}">{{ count(Auth::user()->unreadNotifications) }}</span> --}}
                    data-count="{{ count(auth()->guard('trader')->user()->unreadNotifications) }}">{{ count(auth()->guard('trader')->user()->unreadNotifications) }}</span>
            @endif
        </span>
    </a>
    <!-- Dropdown - Alerts -->
    <div class="dropdown-list dropdown-menu dropdown-menu-right shadow animated--grow-in"
        ...
        @foreach (auth()->guard('trader')->user()->unreadNotifications as $notification)
        ...
        <a class="dropdown-item text-center small text-gray-500" href="{{ route('all.notification') }}">رؤية جميع
            الاشعارات</a>
    </div>
</div>
